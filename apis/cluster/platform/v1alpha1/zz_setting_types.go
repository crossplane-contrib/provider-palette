...
type SettingInitParameters struct {
	...
	// (Boolean) Enables automatic remediation. Allowed only for project context.
	...
	// FIPS-compliant addon packs when creating cluster profiles. The non_fips_addon_pack setting is supported only in Palette Vertex environments. Allowed only for tenant context.
	...
	// compliant. The non_fips_cluster_import setting is supported only in Palette Vertex environments. Allowed only for tenant context.
	...
	// FIPS-compliant features such as backup, restore, and scans. The non_fips_features setting is supported only in Palette Vertex environments. Allowed only for tenant context.
	...
	// (Number) Specifies the duration in minutes of inactivity before a user is automatically logged out. The default is 240 minutes in Palette. Allowed only for tenant context.
...
// Setting is the Schema for the Settings API. Resource for managing tenant and project platform settings in Spectro Cloud.
