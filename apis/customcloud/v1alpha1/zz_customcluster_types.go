...
type CustomClusterInitParameters struct {
	...
	// +crossplane:generate:reference:type=github.com/crossplane-contrib/provider-palette/apis/customcloud/v1alpha1.CustomCloudAccount
	CloudAccountID *string `json:"cloudAccountId,omitempty" tf:"cloud_account_id,omitempty"`

	// Reference to a CustomCloudAccount in customcloud to populate cloudAccountId.
	// +kubebuilder:validation:Optional
	CloudAccountIDRef *v1.Reference `json:"cloudAccountIdRef,omitempty" tf:"-"`

	// Selector for a CustomCloudAccount in customcloud to populate cloudAccountId.
	// +kubebuilder:validation:Optional
...
type CustomClusterSpec struct {
	...
	ForProvider     CustomClusterParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider CustomClusterInitParameters `json:"initProvider,omitempty"`
}

// CustomClusterStatus defines the observed state of CustomCluster.
type CustomClusterStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        CustomClusterObservation `json:"atProvider,omitempty"`
}
...
// CustomCluster is the Schema for the CustomClusters API. Resource for managing custom cloud clusters in Spectro Cloud through Palette.
...
// CustomClusterList contains a list of CustomClusters
...
var (
	CustomCluster_Kind             = "CustomCluster"
	CustomCluster_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: CustomCluster_Kind}.String()
	CustomCluster_KindAPIVersion   = CustomCluster_Kind + "." + CRDGroupVersion.String()
	CustomCluster_GroupVersionKind = CRDGroupVersion.WithKind(CustomCluster_Kind)
)
